...
def get_latest_chapter_number():
    with sync_playwright() as p:
        ...
        page.goto("https://onepiece.tube/manga/kapitel-mangaliste", timeout=TIMEOUT)
        value = page.locator("div.border-bottom").nth(2).locator("p").text_content()
        browser.close()
...
def get_chapter_image_urls(chapter):
    ...
    with sync_playwright() as p:
        ...
        # Seite 1 laden
        page.goto(f"{BASE_CHAPTER_URL}{chapter}/1", timeout=TIMEOUT)
        page.wait_for_selector("img.img-responsive", timeout=TIMEOUT)

        # Seitenanzahl aus Pagination bestimmen
        page_labels = page.locator("ul.nav-pages li a[aria-label]").all()
        pages = []
        ...
        for i in range(1, max_page + 1):
            try:
                page.goto(f"{BASE_CHAPTER_URL}{chapter}/{i}", wait_until="domcontentloaded", timeout=TIMEOUT)
                page.wait_for_selector("img.img-responsive", timeout=TIMEOUT)
...
def save_chapter_as_pdf(chapter_num, img_urls, out_dir=OUT_DIR):
    if not img_urls:
        return
...
def get_all_chapters(first_chapter=START_CHAPTER, last_chapter=MAX_CHAPTER):
    ...
    for chapter in range(first_chapter, last_chapter + 1):
        try:
            img_urls = get_chapter_image_urls(chapter)
            ...
            consecutive_404 = 0
            save_chapter_as_pdf(chapter, img_urls)
...
# ----------------------------------------------------------
# ALT: Raten der Bild-URLs (nicht mehr verwendet)
# ----------------------------------------------------------
# import re
# import requests
# from PIL import Image
